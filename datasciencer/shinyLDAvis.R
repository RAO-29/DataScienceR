...
ui <- fluidPage(
  ...
  br(),
  fluidRow(
    column(4, textInput(inputId = "query",
                        label = NULL,
                        value = NULL,
                        width = "100%",
                        placeholder = "realdonaldtrump,maga,america <- Query")),
    column(1, actionButton(inputId = "goButton",label = "Submit")),
    column(5, offset = 1, helpText("Note: Changes to
          Query, Streamtime or Options are applied after submit only!"))), 
  
  fluidRow(
    column(1, numericInput(inputId = "streamtime",label = NULL,
                           value = 10, min = 5, max = 60, 
                           step = 1, width = "100%" )),
    column(1, checkboxInput(inputId = "infoPopup",
                            label = "Info Popups?",
                            value = TRUE)),
    column(1, checkboxInput(inputId = "batch_processing",
                            label = "Cached Tweets?",
                            value = TRUE)),
    column(2, checkboxInput(inputId = "bot_detection",
                            label = "Detect Bots? (Note: Experimental Feature)",
                            value = FALSE)),
...
server <- function(input, output) {
  ...
  ## Wrapper for input-elements to accomodate 'different types' of 
  ## reactive behaviour, these being execution 'at once' and batch-execution
  reactiveTwitterLDA <- reactive ({
    ...
    if (infoPopup) { showModal(modalDialog(
      title = NULL,
      size = "s",
    ...
    TwitterLDA(streamtime = streamtime, query = query, 
  ...
  runReactive <- reactive({
    ...
    ## Also check if data for Phi matrix exists -> if not, there would be an
    ## error while trying to get the JSON-string for rendering
